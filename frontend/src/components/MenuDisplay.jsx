  ...
  return (
    <div id="menu" className="py-32 px-6 md:px-20 bg-transparent scroll-mt-24">
      <div className="max-w-7xl mx-auto space-y-20">
        ...
        {/* HEADER */}
        <div className="flex flex-col md:flex-row items-baseline gap-6 border-b border-heritage-espresso/5 pb-12">
          <h2 className="text-6xl md:text-8xl font-serif italic text-heritage-espresso leading-none">{t('menu')}</h2>
        ...
        {/* CATEGORY TABS */}
        <div className="flex gap-4 overflow-x-auto no-scrollbar pb-6 sticky top-24 z-40 bg-[var(--bg-primary)]/80 backdrop-blur-md -mx-6 px-6 pt-4">
          {categories.map(cat => (
            <button
              key={cat}
              onClick={() => setActiveCategory(cat)}
              className={`px-10 py-4 rounded-full text-[10px] font-black uppercase tracking-widest transition-all border ${activeCategory === cat ? 'bg-heritage-espresso text-white border-heritage-espresso shadow-2xl' : 'bg-white/50 text-heritage-espresso/40 border-heritage-espresso/5 hover:bg-white'}`}
            >
              {cat}
            </button>
          ))}
          ...
          {Object.entries(groupedMenu).map(([category, items]) => (
            <div key={category} className="space-y-12">
              <div className="flex items-center gap-6">
                <h3 className="text-4xl font-serif italic text-heritage-gold whitespace-nowrap">{category}</h3>
...
function MenuItemCard({ item, onAdd }) {
  ...
  return (
    ...
    >
      <div className="relative aspect-square rounded-[3rem] overflow-hidden bg-heritage-stone shadow-xl border border-heritage-espresso/5">
        <img
      ...
      <div className="space-y-4">
        ...
        <button
          ...
          className={`w-full py-5 rounded-[2rem] text-[10px] font-black uppercase tracking-[0.3em] transition-all flex items-center justify-center gap-4 ${added ? 'bg-green-500 text-white' : item.available === false ? 'bg-heritage-espresso/10 text-heritage-espresso/20 cursor-not-allowed' : 'bg-heritage-gold text-white hover:bg-heritage-espresso shadow-xl shadow-heritage-gold/20'}`}
        >
