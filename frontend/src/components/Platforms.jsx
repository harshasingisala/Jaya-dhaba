  ...
  return (
    <section className="py-32 px-6 md:px-20 bg-heritage-stone/30">
      <div className="max-w-2xl mx-auto text-center mb-20 space-y-6">
         ...
         <p className="text-heritage-espresso/75 text-sm font-medium leading-relaxed italic">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-10 max-w-7xl mx-auto">
        {apps.map((app, i) => (
          ...
          >
            <div className={`w-20 h-20 rounded-[2.5rem] flex items-center justify-center transition-all duration-500 ${app.isInternal ? 'bg-heritage-terracotta text-white' : 'bg-heritage-espresso/5 text-heritage-espresso/75 group-hover:bg-heritage-gold group-hover:text-white'}`}>
              {app.icon}
            </div>
            
            <div className="space-y-4">
              <h3 className="text-2xl font-serif italic text-heritage-espresso tracking-tight">{app.name}</h3>
              <p className="text-heritage-espresso/75 text-[13px] leading-relaxed font-medium">
                {app.desc}
              </p>
            </div>
